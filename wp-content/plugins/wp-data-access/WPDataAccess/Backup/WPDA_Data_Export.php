...
namespace WPDataAccess\Backup {
	...
	class WPDA_Data_Export {
		...
		public function create_export( $action ) {
			...
			?>
			<div class="wrap">
			<h1 class="wp-heading-inline"><span>Data Backups</span></h1>
			<div id="wpda_export_import">
				<div class="wpda_export_import">
					<form id="wpda_export_import_form" action="?page=wpda_backup" method="post" onsubmit="return pre_submit()">
						<table>
							<tr>
								<td style="font-weight:bold;padding-left:10px;">Database Tables</td>
								<td></td>
								<td style="font-weight:bold;padding-left:10px">Tables To Be Exported</td>
							</tr>
							<tr>
								<td>
									<select id="wpda_table_name_db" name="wpda_table_name_db" multiple size="20" style="width:300px;">
										<?php
							...
							<tr>
								<td colspan="3" style="text-align:center;">
									<table align="center">
										<tr>
											<td style="text-align:right;">Backup Id</td>
											<td style="text-align:left;">
												<input
													...
													maxlength="50"
													<?php if ( 'update' === $action ) { echo 'readonly';} ?>
												/>
											</td>
										</tr>
										<tr>
											<td style="text-align:right;">Backup Interval</td>
											<td style="text-align:left;">
												<select id="interval" name="interval">
													<?php if ( 'add' === $action ) { ?>
													<option value="runonce">Run once (no interval)</option>
													<?php } ?>
													<?php
										...
										<tr>
											<td style="text-align:right;">Backup Location</td>
											<td style="text-align:left;">
										...
										<tr>
											<td style="text-align:right;">Backup Files Kept</td>
											<td style="text-align:left;">
			...
			<script language="JavaScript">
				...
				function pre_submit() {
					if ( 0 === jQuery("#wpda_table_name_export > option").size() ) {
						alert('No tables to be exported');
						return false;
					}
					if ( '' === jQuery("#backupid").val().trim() ) {
						alert('You must specify a backupid');
						return false;
					}
		...
		public function show_wp_cron() {
			...
			echo '<div class="wrap">';
			echo '<h1 class="wp-heading-inline"><span>Data Backups</span></h1>';
			echo '<form method="post" action="?page=wpda_backup" style="display: inline-block; vertical-align: unset;">';
			echo '<input type="submit" class="page-title-action" value="New Data Backup" />';
			...
			if ( $data_backups_found ) {
				...
				echo '<th></th>';
				echo '<th></th>';
				echo '<th style="text-align:left;">Hook Name</th>';
				echo '<th style="text-align:left;">Arguments</th>';
				echo '<th style="text-align:left;">Interval</th>';
				echo '<th style="text-align:left;">Next Execution</th>';
				echo '<th style="text-align:left;">Backup Location</th>';
				echo '<th style="text-align:left;">Files Kept</th>';
				echo '<th colspan="2" style="text-align:left;">Status</th>';
				echo '</tr>';
				foreach ( $crons as $key => $cron ) {
					foreach ( $cron as $key => $value ) {
						if ( 'wpda_data_backup' === $key ) {
						...
						if ( 'wpda_data_backup' === $key ) {
							echo '<td>';
							echo '<form method="post" action="?page=wpda_backup">';
						...
						foreach ( $value as $value_key => $value_value ) {
							...
							echo '<td>';
							if ( 'wpda_data_backup' === $key ) {
								echo '<form method="post" action="?page=wpda&table_name=wp_wpda_logging&s=' . $value_value['args'][0] .'">';
							...
							if ( 'wpda_data_backup' === $key ) {
								$sql =
									...
									'   FROM ' . $wpdb->prefix . WPDA::get_option( WPDA::OPTION_WPDA_PREFIX ) . 'logging ' .
								...
								if ( 1 === $wpdb->num_rows ) {
									...
									echo 'No logging information found';
								}
							}
							echo '</td>';
						}
		...
		protected function wpda_data_backup_run( $backupid, $keep, $device, $tables ) {
			try {
				...
				if ( 'local_path' === $device ) {
					...
					// To use a stream for Dropbox we'll first write the export to a temporary file.
		...
		public function wpda_add_cron_job() {
			...
			if ( 'runonce' === $interval ) {
				...
				<div class="wrap">
					<h1 class="wp-heading-inline"><span>Data Backups</span></h1>
					<p>Data backup started. Please check backup location.</p>
					<p><a href="?page=wpda_backup" class="button">Back to main page</a></p>
				</div>
				<?php
			} else {

...
namespace WPDataAccess\Utilities {
	...
	class WPDA_Import_File {
		...
		public function import( $schema_name, $table_name, $hide_errors ) {
			...
			if ( '' === $schema_name || $wpdb->dbname === $this->schema_name ) {
				// Perform insert on WordPress schema.
				$db = null;
				$suppress = $wpdb->suppress_errors( 'on' === $hide_errors );
			} else {
				// Perform insert on other schema.
				$db = new \wpdb( DB_USER, DB_PASSWORD, $schema_name, DB_HOST );

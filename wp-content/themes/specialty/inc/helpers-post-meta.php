<?php
add_action( 'after_setup_theme', 'specialty_setup_helpers_post_meta' );
function specialty_setup_helpers_post_meta() {
	add_image_size( 'specialty_featgal_small_thumb', 100, 100, true );

	add_action( 'wp_ajax_specialty_featgal_AJAXPreview', 'specialty_featgal_AJAXPreview' );
}

add_action( 'admin_enqueue_scripts', 'specialty_admin_register_post_meta_scripts' );
function specialty_admin_register_post_meta_scripts( $hook ) {
	...
	$settings = array(
		...
		'tSelectFile'         => esc_html__( 'Select file', 'specialty' ),
		'tSelectFiles'        => esc_html__( 'Select files', 'specialty' ),
		'tUseThisFile'        => esc_html__( 'Use this file', 'specialty' ),
		'tUseTheseFiles'      => esc_html__( 'Use these files', 'specialty' ),
		'tUpdateGallery'      => esc_html__( 'Update gallery', 'specialty' ),
		'tLoading'            => esc_html__( 'Loading...', 'specialty' ),
		'tPreviewUnavailable' => esc_html__( 'Gallery preview not available.', 'specialty' ),
		'tRemoveImage'        => esc_html__( 'Remove image', 'specialty' ),
		'tRemoveFromGallery'  => esc_html__( 'Remove from gallery', 'specialty' ),
	);
	wp_localize_script( 'specialty-post-meta', 'specialty_PostMeta', $settings );
}
...
function specialty_prepare_metabox( $post_type ) {
	wp_nonce_field( basename( __FILE__ ), $post_type . '_nonce' );
}

function specialty_can_save_meta( $post_type ) {
	global $post;
...
function specialty_metabox_gallery( $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	specialty_featgal_print_meta_html( $post_id, $gid );
}

function specialty_metabox_gallery_save( $POST, $gid = 1 ) {
	global $post;
	$post_id = $post->ID;

	specialty_featgal_update_meta( $post_id, $POST, $gid );
}

function specialty_metabox_input( $fieldname, $label, $params = array() ) {
	global $post;
...
function specialty_metabox_textarea( $fieldname, $label, $params = array() ) {
	global $post;
...
function specialty_metabox_dropdown( $fieldname, $options, $label, $params = array() ) {
	global $post;
...
function specialty_metabox_radio( $fieldname, $optionname, $optionval, $label, $params = array() ) {
	global $post;
...
function specialty_metabox_checkbox( $fieldname, $value, $label, $params = array() ) {
	global $post;
...
function specialty_metabox_close_tab() {
	?>
		</div>
	</div>
	<?php
...
function specialty_metabox_open_collapsible( $title ) {
	?>
	<div class="postbox" style="margin-top:20px">
		<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'specialty' ); ?>"><br></div>
		<h3 class="hndle"><?php echo esc_html( $title ); ?></h3>
		<div class="inside">
	<?php
}

function specialty_metabox_close_collapsible() {
	?>
		</div>
	</div>
	<?php
...
function specialty_metabox_guide( $strings, $params = array() ) {
	$defaults = array(
...
function specialty_bind_metabox_to_page_template( $metabox_id, $template_file, $js_var ) {
...
ENDJS;
	...
	specialty_add_inline_js( $js, sanitize_key( 'metabox_template_' . $metabox_id . '_' . $template_file ) );
}

function specialty_bind_metabox_to_post_format( $metabox_id, $post_format, $js_var ) {
...
ENDJS;
	...
	specialty_add_inline_js( $js, sanitize_key( 'metabox_format_' . $metabox_id . '_' . $post_format ) );
}
...
function specialty_featgal_print_meta_html( $post_id = false, $gid = 1 ) {
	...
	<div class="ci-media-manager-gallery">
		<input type="button" class="ci-upload-to-gallery button" value="<?php esc_attr_e( 'Add Images', 'specialty' ); ?>"/>
		...
		<p><label class="ci-upload-to-gallery-random"><input type="checkbox" name="ci_featured_gallery_rand_<?php echo esc_attr( $gid ); ?>" value="rand" <?php checked( $rand, 'rand' ); ?> /> <?php esc_html_e( 'Randomize order', 'specialty' ); ?></label></p>
				...
				if ( false !== $images && is_array( $images ) ) {
					foreach ( $images as $image ) {
						...
						<div class="thumb">
							...
							<a href="#" class="close media-modal-icon" title="<?php esc_attr_e( 'Remove from gallery', 'specialty' ); ?>"></a>
						</div>
						<?php
					}
				}
			?>
			<p class="ci-upload-to-gallery-preview-text"><?php esc_html_e( 'Your gallery images will appear here', 'specialty' ); ?></p>
		</div>
	</div>
	<?php
}
 ...
 */
function specialty_featgal_update_meta( $post_id, $POST, $gid = 1 ) {
	if ( absint( $post_id ) < 1 ) {
		return false;
	}
 ...
 * Hooked to wp_ajax_specialty_featgal_AJAXPreview for AJAX updating of the galleries' previews.
 */

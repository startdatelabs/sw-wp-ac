jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? specialty_PostMeta.tSelectFiles : specialty_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? specialty_PostMeta.tUseTheseFiles : specialty_PostMeta.tUseThisFile
			},
			multiple: bMulti
		} ).on( 'select', function(){
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						// Only 'thumbnail', 'medium' and 'full' are exposed, so we use 'thumbnail' instead of 'specialty_featgal_small_thumb'
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + specialty_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: specialty_PostMeta.tSelectFiles,
			button: {
				text: specialty_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			specialty_featgal_AJAXUpdate( target_parent );
	...
	function specialty_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function specialty_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: specialty_PostMeta.ajaxurl,
			data: {
				action: 'specialty_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + specialty_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + specialty_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + specialty_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		specialty_featgal_UpdateIDs( container );
	} );
	...
	if ( preview_areas.length > 0 ) {
		preview_areas.sortable( {
			update: function( event, ui ) {
				specialty_featgal_UpdateIDs( $( this ) );
			}
		} );
	}

						...
						<?php while ( have_posts() ) : the_post(); ?>
							<?php
								...
								$jobs = get_job_listings( apply_filters( 'job_manager_output_jobs_args', array(
									...
									'search_categories' => $selected_category,
								...
								$title = sprintf( '<h3 class="section-title"><span class="jobs-found-no">%s</span></h3>',
									esc_html( sprintf( _n( '%d job found', '%d jobs found', $jobs->found_posts, 'specialty' ), $jobs->found_posts ) )
								);
								?>
								...
								<?php if ( ! is_null( $jobs ) && $jobs->have_posts() ) : ?>
